  ...
  <div>
    ...
    >
      <button
        ...
        :class="disablePagination ? 'cursor-not-allowed' : 'hover:bg-gray-200'"
        :disabled="disablePagination"
        @click="prevPage"
      >
        &lt; Previous Page
      </button>
      <span v-if="showPageNumbers" class="font-medium text-gray-800">
        {{ page + 1 }} / {{ totalPages }}
      </span>
      <button
        class="rounded py-1 px-3 transition duration-200 focus:outline-none"
...
export default {
  ...
  setup({ data, initialPage, itemsPerPage }) {
    ...
    const visibleRows = computed(() =>
      data.slice(page.value * itemsPerPage, (page.value + 1) * itemsPerPage)
    );
